...
$select = mysql_query("SELECT * FROM `data_armore`");
...
<body>
    ...
    <br>
    <form action="craft_armore_choise_component.php" method = "post">
        <select name="must_craft">
            <option selected desabled>Choise what create</option>
            <?
            while($element = mysql_fetch_array($select)){
                echo "<option value = '".$element['armore_id']."'>".$element['armore_name']."</option>";
            }
            ?>
        </select>
